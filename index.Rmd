...
For each example, I will use the `mtcars` data set from the `datasets` package. The `mtcars` data set has `r nrow(mtcars)` rows and `r ncol(mtcars)` columns. Feel free to load your own data table or use the `mtcars` data set.
...
Let’s say we have a bunch of text that we want to put side-by-side with this small table. Our subsetted `mtcars` data set now has `r nrow(mtcars_sub)` rows and `r ncol(mtcars_sub)` columns. We can make our table smaller by setting `full_width = FALSE` in `kable_styling` and float the table by setting `position = "float_right"`
...
````
```{r setup, include=FALSE, purl = FALSE}`r ''`
knitr::opts_chunk$set(echo = TRUE, knitr.purl.inline = TRUE)
```
````
...
```{r plot_pressure, fig.cap="<small>***Figure** generated with R code and rendered in the final document.* </small>"}
plot(pressure)
```
...
```{r save_plot_pressure, results='hide'}
...
```{r bad_color, out.width="70%", echo=FALSE, fig.cap="<small>*Example of **color as an enemy**. This figure has way too many colors to convey a clear message. Unfortunately, figures like this are common in the literature*. </small>"}
knitr::include_graphics("files/bad-color.png")
```
...
```{r color_8_tab, echo=FALSE}
text_tbl <- data.frame(
  Color = c("_______", "_______", "_______", 
            "_______", "_______", "_______", 
            "_______", "_______"),
  Name = c("black", "blue", "sky blue", "reddish purple", 
           "bluish green", "vermillion", "orange", "yellow"),
  HEX = c("#000000", "#0072B2", "#56B4E9", "#CC79A7", 
              "#009E73", "#D55E00", "#E69F00", "#F0E442"),
  RGB = c("0, 0, 0", "0, 114, 178", "86, 180, 233", 
               "204, 121, 167", "0, 158, 115", "213, 94, 0", 
               "230, 159, 0", "240, 228, 66")
)

text_tbl$Color <- cell_spec(text_tbl$Color, 
                            format = "html", 
                            background = c("#000000", 
                                           "#0072B2", "#56B4E9", 
                                           "#CC79A7", "#009E73", 
                                           "#D55E00", "#E69F00", 
                                           "#F0E442"), 
                            background_as_tile = TRUE, font_size = 20)

kable(text_tbl, escape = FALSE, caption = "[CONSERVATIVE 8-COLOR PALETTE](color/8-color-pal.txt)") %>%
  kable_styling(bootstrap_options = c("bordered", "condensed"), 
                full_width = F, position = "float_left", font_size = 14) %>%
  column_spec(1, width = "6em", border_right = T) %>%
  column_spec(2, bold = T, border_right = T, border_left = T) %>%
  column_spec(3, bold = T, border_right = T) %>%
  column_spec(4, bold = T, border_right = T)
```


```{r color_12_tab, echo=FALSE}
text_tbl <- data.frame(
  Color = c("_______", "_______", "_______", 
            "_______", "_______", "_______", 
            "_______", "_______", "_______", 
            "_______", "_______", "_______"),
  Name = c("dark grey", "jazzberry jam", "kepel", 
           "light grey", "seance", "mariner", "capri", 
           "pale plum", "saddle brown", "mars yellow", 
           "fuego", "paris daisy"),
  HEX = c("#323232", "#BF3465", "#50B29E", 
              "#D9D9D9", "#731683", "#1C6CCC", 
              "#21BCFF", "#DFA5E5", "#874310", 
              "#DB6D1B", "#B8CE17", "#F4E345"),
  RGB = c("50, 50, 50", "191, 52, 101", "80, 178, 158", 
               "217, 217, 217", "115, 22, 131", "28, 108, 204", 
               "33, 188, 255", "223, 165, 229", "135, 67, 16", 
               "219, 109, 27", "184, 206, 23", "244, 227, 69")
)

text_tbl$Color <- cell_spec(text_tbl$Color, 
                            format = "html", 
                            background = c("#323232", "#BF3465", "#50B29E",
                                           "#D9D9D9", "#731683", "#1C6CCC",
                                           "#21BCFF", "#DFA5E5", "#874310",
                                           "#DB6D1B", "#B8CE17", "#F4E345"),
                            background_as_tile = TRUE, font_size = 20)

kable(text_tbl, escape = FALSE, caption = "[12-COLOR PALETTE](color/12-color-pal.txt)") %>%
  kable_styling(bootstrap_options = c("bordered", "condensed"), 
                full_width = F, position = "right", font_size = 13) %>%
  column_spec(1, width = "6em", border_right = T) %>%
  column_spec(2, bold = T, border_right = T, border_left = T) %>%
  column_spec(3, bold = T, border_right = T) %>%
  column_spec(4, bold = T, border_right = T)
...
text_tbl <- data.frame(
  Color = c("_______", "_______", "_______", 
            ...
            "_______", "_______", "_______", 
            "_______"),
  Name = c("black", "blue lagoon", "seance", 
           "science blue", "azure radiance", "blush pink", 
           "bright turquoise", "shiraz", "salem", 
           "salmon", "ronchi", "mint green", 
           "dairy cream"),
  HEX = c("#000000", "#006E82", "#8214A0", 
              "#005AC8", "#00A0FA", "#FA78FA", 
              "#14D2DC", "#AA0A3C", "#0A9B4B", 
              "#FF825F", "#EAD644", "#A0FA82", 
              "#FAE6BE"),
  RGB = c("0, 0, 0", "0, 110, 130", "130, 20, 160", 
               "0, 90, 200", "0, 160, 250", "250, 120, 250", 
               "20, 210, 220", "170, 10, 60", "10, 155, 75", 
               "255, 130, 95", "234, 214, 68", "160, 250, 130", 
               "250, 230, 190")
)
text_tbl$Color <- cell_spec(text_tbl$Color, 
                            format = "html", 
                            background = c("#000000", "#006E82", 
                                           "#8214A0", "#005AC8", "#00A0FA",
                                           "#FA78FA", "#14D2DC", "#AA0A3C",
                                           "#0A9B4B", "#FF825F", "#EAD644",
                                           "#A0FA82", "#FAE6BE"),
                            background_as_tile = TRUE, font_size = 20)

kable(text_tbl, escape = FALSE, caption = "[13-COLOR PALETTE](color/13-color-pal.txt)") %>%
  kable_styling(bootstrap_options = c("bordered", "condensed"), 
                full_width = F, position = "float_left", font_size = 13) %>%
  column_spec(1, width = "6em", border_right = T) %>%
  column_spec(2, bold = T, border_right = T, border_left = T) %>%
  column_spec(3, bold = T, border_right = T) %>%
  column_spec(4, bold = T, border_right = T)
...
text_tbl <- data.frame(
  Color = c("_______", "_______", "_______", 
            ...
            "_______", "_______", "_______",
            "_______", "_______", "_______"),
  Name = c("black", "sherpa blue", "persian green", 
           "hot pink", "cotton candy", "pigment indigo", 
           "science blue", "heliotrope", "malibu", 
           "french pass", "red berry", "brown", 
           "mango tango", "harlequin", "laser lemon"),
  HEX = c("#000000", "#004949", "#009292", 
              "#ff6db6", "#ffb6db", "#490092", 
              "#006ddb", "#b66dff", "#6db6ff", 
              "#b6dbff", "#920000", "#924900", 
              "#db6d00", "#24ff24", "#ffff6d"),
  RGB = c("0, 0, 0", "0 73 73", "0 146 146", 
               "255, 109, 182", "255, 182, 219", "73 0 146", 
               "0, 109, 219", "182 109 255", "109, 182, 255", 
               "182 216 255", "146, 0, 0", "146 73 0", 
               "219, 109, 0", "36, 255, 36", "255, 255, 109")
)

text_tbl$Color <- cell_spec(text_tbl$Color, 
                            format = "html", 
                            background = c("#000000", "#004949", "#009292", 
                                           "#ff6db6", "#ffb6db", "#490092", 
                                           "#006ddb", "#b66dff", "#6db6ff", 
                                           "#b6dbff", "#920000", "#924900", 
                                           "#db6d00", "#24ff24", "#ffff6d"),
...
kable(text_tbl, escape = FALSE, caption = "[15-COLOR PALETTE](color/15-color-pal.txt)") %>%
  kable_styling(bootstrap_options = c("bordered", "condensed"), 
                full_width = F, position = "right", font_size = 12) %>%
  column_spec(1, width = "6em", border_right = T) %>%
  column_spec(2, bold = T, border_right = T, border_left = T) %>%
  column_spec(3, bold = T, border_right = T) %>%
  column_spec(4, bold = T, border_right = T)
...
```{r color_121, fig.height=6, out.width = '60%', fig.align='center', comment='', fig.show='hold', eval=FALSE, echo=FALSE}
# Wong palette
wong_pal <- c("#009E73", "#D55E00", "#F0E442",
                "#CC79A7", "#56B4E9", "#E69F00",
                "#0072B2", "#7F7F7F", "#000000")
...
twelve_pal <- c("#323232", "#BF3465", "#50B29E", 
                "#D9D9D9", "#731683", "#1C6CCC", 
                "#21BCFF", "#DFA5E5", "#874310", 
                "#DB6D1B", "#B8CE17", "#F4E345")
#show_col(twelve_pal)

thirteen_pal <- c("#000000", "#006E82", "#8214A0", 
                  "#005AC8", "#00A0FA", "#FA78FA", 
                  "#14D2DC", "#AA0A3C", "#0A9B4B", 
                  "#FF825F", "#EAD644", "#A0FA82", 
                  "#FAE6BE")
#show_col(thirteen_pal)

fifteen_pal <- c("#000000", "#004949", "#009292", 
              "#ff6db6", "#ffb6db", "#490092", 
              "#006ddb", "#b66dff", "#6db6ff", 
              "#b6dbff", "#920000", "#924900", 
              "#db6d00", "#24ff24", "#ffff6d")
show_col(fifteen_pal)
```
...
```{r out_2, fig.align='center', fig.width=8, fig.height=4, fig.cap='This is a figure caption.', echo=FALSE}
plot(cars, main = "Plot Title", 
     xlab = "temperature (C)", 
     ylab = "pressure (Pa)", 
     pch = 24, cex=1.5, col="red", bg="blue", lwd=2)
...
````
```{r grid, fig.show='hold', out.width='33%'}`r ''`
p1 <- plot(pressure)
p2 <- plot(cars)
p3 <- plot(pressure)
```
...
```{r grid, echo=FALSE, fig.show='hold', out.width='33%'}
p1 <- plot(pressure)
p2 <- plot(cars)
p3 <- plot(pressure)
```
